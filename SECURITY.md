...
- Unix files are created and reverified with mode `0600`. The destination directory must be owned by the current effective user; group- or world-writable directories are rejected unless the sticky bit is set. Windows files are created with a protected DACL granting full control only to the owner, Local System, and local administrators, and that DACL is reverified before and after persistence. Windows runtime behavior has not yet been observed in this repository's hosted CI, and administrator, backup-privilege, non-NTFS, or network-share behavior remains outside the guarantee.
